...
__all__ = ["available", "ideal_atoms", "build_ligand_model",
           ...
           "fit_into_density", "unknown_ligands", "restraints_from_residue",
           "apply_generated_restraints"]
...
def _provenance_header(code: str, provenance: dict, date: str) -> List[str]:
    ...
    # Two ways in, and the header must not claim the wrong one: a SMILES the user typed
    # is a statement of what the ligand *is*, while one read back out of coordinates is a
    # guess that can be wrong, and anyone judging whether to trust these restraints needs
    # to know which they are holding.
    from_coordinates = provenance.get("derived_from", "").startswith("model coordinates")
    ...
    if from_coordinates:
        method = [
            ...
            "#                 thing being fixed. Nominal esds (bond 0.020 A, angle 3.0 deg).",
            "# Check         : the perceived SMILES above is a guess about chemistry the",
            "#                 model file never stated. Confirm it before refining.",
        ]
    else:
        method = [
    ...
    lines = [
        ...
        f"# {smiles_label}: {provenance.get('input_smiles', '(unknown)')}",
...
def unknown_ligands(model: Any) -> List[dict]:
    """Residues whose atoms cctbx cannot assign an energy type to.

    Asked of cctbx rather than worked out here: ``pdb_interpretation`` builds a registry
    of nonbonded energy types while it interprets the file and records exactly which atoms
    it could not place, so this reads that registry instead of second-guessing what the
    monomer library contains. Checking a residue name against the library is *not*
    equivalent -- a code the library knows still fails when the model's atom names do not
    match the entry's, which is a common way for a hand-edited ligand to break.

    Returns one dict per residue: ``{"code", "chain", "resseq", "i_seqs", "n_atoms"}``.
    Empty when the model interprets cleanly, which is the usual case and costs one
    interpretation pass to establish.
    """
    ...
    pdb_inp = iotbx.pdb.input(
        source_info=None,
        lines=hierarchy.as_pdb_string(crystal_symmetry=model.crystal_symmetry()).splitlines())

    # Any dictionary the model already carries is registered first, or a ligand that has
    # just been given restraints would be reported as still needing them.
    mon_lib_srv = mmtbx.monomer_library.server.server()
    for file_name, cif_object in (model.get_restraint_objects() or []):
        try:
            mon_lib_srv.process_cif_object(cif_object, file_name=file_name)
        except Exception:  # pragma: no cover - a dictionary cctbx will reject anyway
            pass

    with open(os.devnull, "w") as devnull:
        processed = pdb_interpretation.process(
            mon_lib_srv=mon_lib_srv,
            ener_lib=mmtbx.monomer_library.server.ener_lib(),
            pdb_inp=pdb_inp,
            log=devnull)
    chain_proxies = processed.all_chain_proxies
    registry = chain_proxies.nonbonded_energy_type_registry
    atoms = registry.get_unknown_atoms(chain_proxies.pdb_atoms, return_iseqs=True)
    if atoms is None or not len(atoms):
        return []

    # The registry flags the atoms it could not type, which is usually only part of the
    # residue -- hydrogens often type fine while their heavy atoms do not. A dictionary
    # has to describe the whole residue, and perceiving chemistry from a subset of a
    # molecule fails outright, so each flagged atom is widened to its whole atom group.
    hierarchy_atoms = model.get_hierarchy().atoms()
    grouped: dict = {}
    for i_seq in atoms:
        group = hierarchy_atoms[int(i_seq)].parent()
        residue = group.parent()
        key = (residue.parent().id.strip(), residue.resseq.strip(), group.resname.strip())
        grouped.setdefault(key, set()).update(
            int(a.i_seq) for a in group.atoms())
    return [{"chain": chain, "resseq": resseq, "code": code,
             "i_seqs": sorted(i_seqs), "n_atoms": len(i_seqs)}
            for (chain, resseq, code), i_seqs in sorted(grouped.items())]


def restraints_from_residue(model: Any, i_seqs: List[int], code: str) -> Tuple[str, str]:
    """``(cif_text, smiles)`` -- a monomer dictionary for one residue, read off its atoms.

    rdkit infers the bonds from interatomic distances and then the bond orders from the
    ...
    **The ideal values do not come from the model's own coordinates.** They are measured
    ...
    The names in the dictionary are the model's own, so cctbx matches the two without the
    ...
    hierarchy_atoms = model.get_hierarchy().atoms()
    selected = list(i_seqs)
    if not selected:
        raise ValueError("no atoms given for %s" % code)

    block = _residue_pdb_block(model, selected)
    mol = Chem.MolFromPDBBlock(block, removeHs=False, proximityBonding=True, sanitize=False)
    if mol is None or mol.GetNumAtoms() != len(selected):
        raise ValueError("rdkit could not read %s back from its coordinates" % code)
    try:
        rdDetermineBonds.DetermineBondOrders(mol, charge=0)
        Chem.SanitizeMol(mol)
    except Exception as exc:
        raise ValueError(
            "rdkit could not work out the chemistry of %s from its coordinates: %s"
            % (code, exc)) from exc

    smiles = Chem.MolToSmiles(Chem.RemoveHs(mol))

    ideal = Chem.Mol(mol)                       # same atom order, clean geometry
    if AllChem.EmbedMolecule(ideal, randomSeed=0xf00d) != 0:
    ...
    elements = [hierarchy_atoms[i].element.strip() for i in selected]
    conformer = ideal.GetConformer()
    xyz = np.array([[conformer.GetAtomPosition(i).x, conformer.GetAtomPosition(i).y,
                     conformer.GetAtomPosition(i).z]
                    for i in range(ideal.GetNumAtoms())])

    provenance = _rdkit_provenance(ideal, smiles)
    provenance["derived_from"] = "model coordinates (bond perception)"
    return _monomer_cif_text(ideal, code, names, elements, xyz, provenance), smiles


def _residue_pdb_block(model: Any, i_seqs: List[int]) -> str:
    """A PDB block holding just these atoms, for rdkit to read.

    Written out rather than handed over as objects because rdkit's reader is what does the
    proximity bonding, and it takes a file format. cctbx does the formatting: a PDB line
    hand-assembled here was wrong by one column -- the altloc field -- which rdkit read as
    a malformed element and turned into an atom bonded to nothing.
    """
    from scitbx.array_family import flex

    selected = model.select(flex.size_t([int(i) for i in i_seqs]))
    return selected.get_hierarchy().as_pdb_string()


def apply_generated_restraints(model: Any, dictionaries: dict) -> None:
    """Attach generated monomer dictionaries to ``model`` so its next build sees them.

    ``dictionaries`` maps residue code to CIF text. They are added to whatever restraint
    objects the model already carries rather than replacing them, so generating one
    ligand's dictionary does not discard another's.
    """
    import iotbx.cif

    existing = list(model.get_restraint_objects() or [])
    known = {name for name, _obj in existing}
    for code, cif_text in sorted(dictionaries.items()):
        filename = "%s.cif" % code
        if filename in known:
            continue
        existing.append((filename, iotbx.cif.reader(input_string=cif_text).model()))
    model.set_restraint_objects(existing)
