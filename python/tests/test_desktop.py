...
def test_volume_commands_go_to_a_session_the_viewport_is_connected_to(qapp):
    ...
    connected to — only the *visible* models. Hiding drops the model from the page (reload),
...
def test_software_pins_a_model_and_says_why_on_click(qapp):
    ...
    and a click on the box flashes why. Both hide on hardware."""
    pytest.importorskip("websockets")
    pytest.importorskip("PySide6.QtWebEngineWidgets")
...
def test_hiding_a_model_reloads_the_scene_without_it(qapp):
    ...
    no in-place GPU mutation (both segfault this app's WebGL). Its socket leaves the page."""
    pytest.importorskip("websockets")
    pytest.importorskip("PySide6.QtWebEngineWidgets")
