...
def _control_session_is_reachable(app) -> None:
    """Volume commands ride the control session, and the page connects to every model's
    socket (hidden ones are just drawn empty) or the dummy when there is no model. A
    control session the page is not listening on means every volume control silently does
    nothing — the hidden-model bug."""
