#!/usr/bin/env bash
# Wire the active conda env to the monomer library that the chem_data package
# ships, so cctbx can build geometry restraints (minimization, tugging, the
# geometry tables, validation) without a separately-managed geostd checkout.
#
# `environment.yml` installs chem_data, but cctbx finds the library through the
# MMTBX_CCP4_MONOMER_LIB env var, which a package cannot set. This writes an
# activate.d hook that points it at geostd inside the env — run once after
# `conda env create`, with the pxviewer env active:
#
#   conda activate pxviewer
#   ./scripts/setup_monomer_lib.sh
#   conda deactivate && conda activate pxviewer   # pick up the hook
set -euo pipefail

if [[ -z "${CONDA_PREFIX:-}" ]]; then
  echo "no active conda env — run: conda activate pxviewer" >&2
  exit 1
fi

geostd="$CONDA_PREFIX/lib/python3.12/site-packages/chem_data/geostd"
if [[ ! -f "$geostd/a/data_ALA.cif" ]]; then
  echo "chem_data's geostd is not at $geostd" >&2
  echo "install it first: conda install -c chem_data chem_data" >&2
  exit 1
fi

hook_dir="$CONDA_PREFIX/etc/conda/activate.d"
mkdir -p "$hook_dir"
cat > "$hook_dir/pxviewer-monomer-lib.sh" <<'EOF'
# pxviewer: point cctbx at the monomer library shipped by the chem_data package.
# Resolved from $CONDA_PREFIX so it stays correct if the env is recreated elsewhere.
export MMTBX_CCP4_MONOMER_LIB="$CONDA_PREFIX/lib/python3.12/site-packages/chem_data/geostd"
EOF

echo "wrote $hook_dir/pxviewer-monomer-lib.sh"
echo "re-activate the env to apply: conda deactivate && conda activate pxviewer"
