...
function App() {
    const model = useCreatePluginViewModel({
        spec: (s) => {
            // Keep Mol*'s click-to-focus representation available, but start with all of its
            // visual components disabled. Settings can opt back into the native target +
            // 5 Å surroundings ball-and-stick view at runtime. Its interaction component is
            // deliberately governed separately by the model's Show menu.
            // Camera focus and focus state remain Mol* defaults independently of this.
